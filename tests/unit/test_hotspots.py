...
class TestGetHotspots:
    ...
    def test_returns_sorted_by_risk(self, tmp_path):
        ...
        result = analyze_complexity(repo="test/repo", storage_path=str(tmp_path))
    ...
    def test_sort_by_cyclomatic(self, tmp_path):
        ...
        _make_store(tmp_path, symbols)
        result = analyze_complexity(repo="test/repo", sort_by="complexity", storage_path=str(tmp_path))
    ...
    def test_sort_by_cognitive(self, tmp_path):
        ...
        _make_store(tmp_path, symbols)
        result = analyze_complexity(repo="test/repo", sort_by="cognitive", storage_path=str(tmp_path))
        assert "b" in result["hotspots"][0]["name"]
    ...
    def test_path_filter(self, tmp_path):
        ...
        _make_store(tmp_path, symbols)
        result = analyze_complexity(repo="test/repo", path="src/", storage_path=str(tmp_path))
        names = [h["name"] for h in result["hotspots"]]
        assert any("\na\n" in n for n in names)
        assert not any("\nb\n" in n for n in names)
    ...
    def test_limit(self, tmp_path):
        ...
        _make_store(tmp_path, symbols)
        result = analyze_complexity(repo="test/repo", limit=3, storage_path=str(tmp_path))
...
class TestGetHotspotsPathValidation:
    """Path parameter validation for analyze_complexity and get_key_symbols."""

    def test_hotspots_rejects_path_traversal(self, tmp_path):
        from codesight_mcp.tools.analyze_complexity import analyze_complexity
        result = analyze_complexity(repo="test/repo", path="../etc/passwd", storage_path=str(tmp_path))
        assert "error" in result
        assert "traversal" in result["error"]

    def test_hotspots_rejects_null_bytes_in_path(self, tmp_path):
        from codesight_mcp.tools.analyze_complexity import analyze_complexity
        result = analyze_complexity(repo="test/repo", path="src/\x00evil", storage_path=str(tmp_path))
        assert "error" in result
        assert "null" in result["error"]

    def test_hotspots_rejects_mid_path_traversal(self, tmp_path):
        from codesight_mcp.tools.analyze_complexity import analyze_complexity
        result = analyze_complexity(repo="test/repo", path="src/../../etc", storage_path=str(tmp_path))
        assert "error" in result
        assert "traversal" in result["error"]
...
def test_analyze_complexity_rejects_invalid_sort_by(tmp_path):
    """analyze_complexity should reject sort_by values not in the allowlist."""
    from codesight_mcp.tools.analyze_complexity import analyze_complexity
    ...
    _make_store(tmp_path, symbols)
    result = analyze_complexity(repo="test/repo", sort_by="<script>", storage_path=str(tmp_path))
    assert "error" in result
