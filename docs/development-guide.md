...
# Run all tests (2,637 tests)
uv run pytest --tb=short -q

# Run specific test categories
uv run pytest tests/unit/              # Parser & analysis (353 tests)
uv run pytest tests/security/          # Adversarial & security (563 tests)
uv run pytest tests/tools/             # Tool functions (345 tests)
uv run pytest tests/server/            # Server & registry (73 tests)
uv run pytest tests/core/              # Core infrastructure (85 tests)
uv run pytest tests/storage/           # Storage persistence (44 tests)
uv run pytest tests/integration/       # End-to-end pipeline (56 tests)
